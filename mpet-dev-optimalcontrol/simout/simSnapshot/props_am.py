...
class Dfuncs():
    ...
    def graphite(self, y):
       # Fitted from Persson, et al. 2010 PRB.
       # Used in Gao, Han, et al.    2021 Joule.
       # Note: This function returns dimensional diffusivity, m^2/s.
       #       Set D=1.0 in param_electrode.cfg
       # Make sure the return is in same type as the input y.

       ln10 = 2.302585092994046
       values    = y
       # More crowd near c=0.5, 1.0
       #values[:] = [ y[k] * 10**(-4.0) * np.exp( ln10 * (-8*y[k]-6)  )\
       #              if y[k] < 0.50 else\
       #              y[k] * 10**(-4.0) * np.exp( ln10 * (-5*y[k]-5))\
       #              for k in range(len(y)) ]

       # Avoid deadlock at c=0.5 and 1.0
       values[:] = [ y[k] * 10**(-4.0) * np.exp( ln10 * (-6.5*y[k]-6.2)  )\
                     if y[k] < 0.50 else\
                     y[k] * 10**(-4.0) * np.exp( ln10 * (-3*y[k]-6.4))\
                     for k in range(len(y)) ]
...
class muRfuncs():
    ...
    def graphite_1param_homog_3(self, y, T, Omga, Omgb, ISfuncs=None):
        ...
        muLlin = (0.15*Omga*12*(0.40-y**0.98)
                  * step_down(y, 0.49, 0.9*width)*step_up(y, 0.35, width))
        muRlin = (3.0*0.1*Omga*4*(0.74-y) + 0.90*Omgb)*step_up(y, 0.5, 0.4*width)
        muLMod = (0.
                  + 40*(-np.exp(-y/0.015))
                  + 0.75*(np.tanh((y-0.17)/0.02) - 1)
                  + 1.0*(np.tanh((y-0.22)/0.040) - 1)
