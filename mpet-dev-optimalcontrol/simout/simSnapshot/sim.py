...
class SimMPET(dae.daeSimulation):
    ...
    def SetUpVariables(self):
        ...
        if not config["prevDir"] or config["prevDir"] == "false":
            ...
            for tr in config["trodes"]:
                ...
                for i in range(Nvol[tr]):
                    ...
                    for j in range(Npart[tr]):
                        ...
                        #GG missing from Huada?
                        if tr =='a': #anode
                                part.Rxn_pl.SetInitialGuess(data[partStr+'Rxn_pl'][-1,i])
                                part.Rxn_chem.SetInitialGuess(data[partStr+'Rxn_chem'][-1,i])
                                part.Atilde.SetInitialGuess(data[partStr+'Atilde'][-1,i])
                                part.VLi.SetInitialCondition(data[partStr+'VLi'][-1,i])
                                part.dVLidt.SetInitialGuess(data[partStr+'dVLidt'][-1,i])
                                part.platingState.AssignValue(data[partStr+'platingState'][-1,i])
                                if data[partStr+'platingState'][-1,i]==0:
                                    part.stnSideReaction.ActiveState = 'NoPlating'
                                if data[partStr+'platingState'][-1,i]==1:
                                    part.stnSideReaction.ActiveState = 'Nucleation'
                                if data[partStr+'platingState'][-1,i]==2:
                                    part.stnSideReaction.ActiveState = 'Growth'
                                
                        part.intercalation_switch.SetInitialGuess(data[partStr+'intercalation_switch'][-1,i])
            ...
            #set last values #GG
            self.m.last_current.AssignValue(utils.get_dict_key(data, "current", final = True))
            self.m.last_phi_applied.AssignValue(utils.get_dict_key(data, "phi_applied", final = True)) 
            ...
            if 4<=laststate<5:
                stnId=str(int(laststate*1e4))
                length = len(stnId)
                partId=stnId[length - 1 :]
                self.m.stnRegulator.ActiveState='LPOverpot_limited_'+partId
            if 5<=abs(laststate)<6:
                stnId=str(int(laststate*1e4))
                length = len(stnId)
                volId=stnId[length - 3 :length - 2 ]
