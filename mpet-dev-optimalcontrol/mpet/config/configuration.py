...
class Config:
    ...
    def _process_config(self, prevDir=None):
        ...
        if self['profileType'] == "BBcontrol":
            self._scale_control_constraints(Vref,theoretical_1C_current)#GG
    ...
    def _scale_control_constraints(self, Vref,theoretical_1C_current):   #GG
        
        factor = constants.e / (constants.k * constants.T_ref)
         
        #Nondimensionalize control constraints
        self["I_ub"] = self['I_ub']*self["1C_current_density"] / (theoretical_1C_current * self['curr_ref'])
        self["V_ub"] = -(factor * self['V_ub'] + Vref)
        self["P_ub"] = self['P_ub']/(self['power_ref'])
        self["ffrac_Tend"] = self["ffrac_Tend"] 
        self["Ce_lb"] = self["Ce_lb"]/ constants.c_ref
        self["Ce_ub"] = self["Ce_ub"]/ constants.c_ref
        self["Cs_ub"] = self["Cs_ub"]/self['a', 'cs_ref']
        self["T_ub"] = self["T_ub"]/ constants.T_ref
        self["LPOverpot_lb"] = self['LPOverpot_lb']*factor;
        ...
        for trode in self['trodes']:
            ...
            if not np.all(self['specified_psd'][trode]):
                ...
                if np.allclose(stddev, 0., atol=1e-12):
                    ...
                    distribution = self['distribution'][trode]
                    if distribution == 'Lognormal':
                        ...
                        # Weibull distributed
                        lbda  = self['mean'][trode]
                        shape = self['stddev'][trode]
                        raw = lbda*np.random.weibull(shape, size=(Nvol, Npart))
                    else:
        ...
        for trode in self['trodes']:
            ...
            for i in range(Nvol):
                for j in range(Npart):
                    ...
                    # Huada:very dirty code
                    if trode=='a':
                        self[trode, 'indvPart']['k0_nuc'][i, j] = self[trode, 'k0_nuc'] \
                            / (constants.e * F_s_ref)
                        self[trode, 'indvPart']['k0_pl'][i, j] = self[trode, 'k0_pl'] \
                            / (constants.e * F_s_ref)
                        self[trode, 'indvPart']['V_nuc'][i, j] = self[trode, 'V_nuc'] \
                            / (constants.k * constants.T_ref / constants.e )
                        # Huada: This quantity is dimensional: m
                        self[trode, 'indvPart']['r_crit'][i, j] = 2.0 * self[trode, 'gamma'] * constants.Omega_Li \
                            / (constants.F * self[trode, 'V_nuc'] )
                        self[trode, 'indvPart']['Vol_ref'][i, j] = cs_ref_part * pvol * constants.Omega_Li / constants.N_A
                        self[trode, 'indvPart']['Vol_Li_ref'][i, j] = \
                                ( 2.0 / 3.0 )* self[trode, 'indvPart']['r_crit'][i,j] * parea\
                                / self[trode, 'indvPart']['Vol_ref'][i,j]
                        self[trode, 'indvPart']['k0_chem'][i, j] = self[trode, 'k0_chem'] \
                            / (constants.e * F_s_ref)
                    self[trode, 'indvPart']['delta_L'][i, j]  = ( parea * plen ) / pvol
                    self[trode, 'indvPart']['parea'][i, j] = parea
