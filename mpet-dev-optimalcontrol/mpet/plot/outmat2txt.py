...
VLiHdrP1 = ("Volume of plated lithium, scale: csmax * Vparticle * Omega_Li / N_A.\n" + RowsStr)
VLiHdrP2 = """Columns correspond to particles specified by simulated
volume index / particle index within volume.
"""
VLiHdrBase = VLiHdrP1 + VLiHdrP2 + seeDiscStr + "\n"
...
def main(indir, genData=True, discData=False, elyteData=False,
         csldData=False, cbarData=False, VLiData=False, bulkpData=False, tempData=False):
    ...
    if VLiData:
            if "a" in trodes:
                VLiDict = plot_data.show_data(
                indir, plot_type="VLi", print_flag=False,
                save_flag=False, data_only=True)
                fname = "VLi{l}Data.txt".format(l="Anode")
                Nv, Np = config["Nvol"]["a"], config["Npart"]["a"]
                NpartTot = Nv*Np
                VLiMat = np.zeros((ntimes, NpartTot))
                VLiHdr = VLiHdrBase
                partInd = 0
                for i in range(Nv):
                    for j in range(Np):
                        VLiHdr += "{i}/{j},".format(j=j, i=i)
                        VLiMat[:,partInd] = VLiDict["a"][:,i,j]
                        partInd += 1
                np.savetxt(os.path.join(indir, fname), VLiMat,
                           delimiter=dlm, header=VLiHdr.rstrip(','))
