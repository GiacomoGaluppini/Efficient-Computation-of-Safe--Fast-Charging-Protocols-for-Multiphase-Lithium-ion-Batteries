...
class SimMPET(dae.daeSimulation):
    ...
    def SetUpVariables(self):
        ...
        if not config["prevDir"] or config["prevDir"] == "false":
                ...
                for i in range(Nvol[tr]):
                    ...
                    for j in range(Npart[tr]):
                        ...
                        if solidType in constants.one_var_types:
                            ...
                            if tr =='a': #anode
                                part.Rxn_pl.SetInitialGuess(0.0)
                                part.Rxn_chem.SetInitialGuess(0.0)
                                part.Atilde.SetInitialGuess(1.0)
                                part.VLi.SetInitialCondition(0.0)
                                part.dVLidt.SetInitialGuess(0.0)
                                part.stnSideReaction.ActiveState = "NoPlating"
                                #GG missing from Huada?
                                part.platingState.AssignValue(0.0)
                            part.intercalation_switch.SetInitialGuess(1.0)
                            for k in range(Nij):
            ...
            self.m.time_counter.AssignValue(0)
            self.m.last_constraint.AssignValue(0)                        

        else: # IF config["prevDir"] IS TRUE
            dPrev = self.dataPrev
            ...
            for tr in config["trodes"]:
                ...
                for i in range(Nvol[tr]):
                    ...
                    for j in range(Npart[tr]):
                        ...
                        #GG missing from Huada?
                        if tr =='a': #anode
                                part.Rxn_pl.SetInitialGuess(data[partStr+'Rxn_pl'][-1,-1])
                                part.Rxn_chem.SetInitialGuess(data[partStr+'Rxn_chem'][-1,-1])
                                part.Atilde.SetInitialGuess(data[partStr+'Atilde'][-1,-1])
                                part.VLi.SetInitialCondition(data[partStr+'VLi'][-1,-1])
                                part.dVLidt.SetInitialGuess(data[partStr+'dVLidt'][-1,-1])
                                part.platingState.AssignValue(data[partStr+'platingState'][-1,-1])
                                if data[partStr+'platingState'][-1,-1]==0:
                                    part.stnSideReaction.ActiveState = 'NoPlating'
                                if data[partStr+'platingState'][-1,-1]==1:
                                    part.stnSideReaction.ActiveState = 'Nucleation'
                                if data[partStr+'platingState'][-1,-1]==2:
                                    part.stnSideReaction.ActiveState = 'Growth'
                                
                        part.intercalation_switch.SetInitialGuess(data[partStr+'intercalation_switch'][-1,-1])
            ...
            #set last values #GG
            self.m.last_current.AssignValue(utils.get_dict_key(data, "last_current", final = True))
            self.m.last_phi_applied.AssignValue(utils.get_dict_key(data, "last_phi_applied", final = True)) 
            self.m.time_counter.AssignValue(utils.get_dict_key(data, "time_counter", final = True))
            self.m.last_constraint.AssignValue(utils.get_dict_key(data, "last_constraint", final = True)) 
            #GG
            laststate=utils.get_dict_key(data, "last_constraint", final = True);
                
            if laststate==0:
                self.m.stnRegulator.ActiveState='startup'
            if laststate==1:
                self.m.stnRegulator.ActiveState='I_limited'
            if laststate==2:
                self.m.stnRegulator.ActiveState='V_limited'
            if laststate==3:
                self.m.stnRegulator.ActiveState='P_limited' 
            if 4<=laststate<5:
                stnId=str(int(np.ceil(laststate*1e4)))
                length = len(stnId)
                partId=stnId[length - 2 :length]
                if partId[0]=='0':
                    partId=partId[1]
                self.m.stnRegulator.ActiveState='LPOverpot_limited_'+partId
            if 5<=abs(laststate)<6:
                stnId=str(int(np.ceil(laststate*1e4)))
                length = len(stnId)
                volId=stnId[length - 4 :length - 2 ]
                if volId[0]=='0':
                    volId=volId[1]
                if laststate<0:#anode
                    self.m.stnRegulator.ActiveState='Ce_lowerlimited_'+volId
                else:#cathode
                    self.m.stnRegulator.ActiveState='Ce_upperlimited_'+volId
            if 6<=laststate<7:
                stnId=str(int(np.ceil(laststate*1e4)))
                length = len(stnId)
                volId=stnId[length - 4 :length - 2 ]
                ...
                if partId[0]=='0':
                    partId=partId[1]
                self.m.stnRegulator.ActiveState="Cs_limited_"+volId+"_"+partId
            if laststate==7:
                self.m.stnRegulator.ActiveState='T_limited' 
                
            print('Active Operating Mode: '+self.m.stnRegulator.ActiveState+'\n')                     
