...
class ModCell(dae.daeModel):
    def __init__(self, config, Name, Parent=None, Description=""):
        ...
        self.last_constraint = dae.daeVariable(
            "last_constraint", dae.no_t, self, "tracks the last active constraint, used for BBcontrol")
...
################################################################################################
        ...
        if self.profileType == "BBcontrol":#GG
            ...
            deltatmin=dae.Constant(0.1*s)
            time_cond = (dae.Time() > deltatmin + self.time_counter())
            ...
            #Define average temperature and average temperature derivative
            T_a=utils.get_asc_vec(self.T_lyte, Nvol, dt=False)
            Tavg=(sum(T_a))/(Nvol['a']+Nvol['s']+Nvol['c'])
             
            dT_a=utils.get_asc_vec(self.T_lyte, Nvol, dt=True)
            dTavg=(sum(dT_a*dae.Constant(1*s)))/(Nvol['a']+Nvol['s']+Nvol['c'])
            ...
            dCe_mindt= dCedt[0]*dae.Constant(1*s)
            dCe_maxdt= dCedt[-1]*dae.Constant(1*s)
            
            #Define Power
            power = self.current()*(self.phi_applied() + ndDVref)
            ...
            if config["tramp"]>0:
                ...
                currentfcn=self.last_current()+(config["I_ub"]-self.last_current())*(1-np.exp(-(dae.Time()-self.time_counter())/dae.Constant(1*s)/tau))
                eq.Residual = self.current() - currentfcn
            else:
                eq.Residual = self.current() - config["I_ub"]
            self.ON_CONDITION((self.phi_applied() <= config["V_ub"]*dbf_plus) & time_cond,#  & (self.last_constraint()!=2),
                              switchToStates=[('Regulator','V_limited')],
                              setVariableValues = [(self.time_counter, dae.Time()),
                                                    (self.last_current, self.current()),
                                                    (self.last_phi_applied, self.phi_applied()),
                                                    (self.last_constraint,2)])
            self.ON_CONDITION((power >= config["P_ub"]*dbf_plus) & time_cond,#  & (self.last_constraint()!=3),
            ...
            for pInd in range(Npart['a']):
                    self.ON_CONDITION((self.particles['a'][Nvol['a']-1,pInd].etaPlating() <=  config["LPOverpot_lb"]*dbf_minus) & time_cond,# & (self.last_constraint()!=4),
            ...
            for vInd in range(Nvol['a']):
                    self.ON_CONDITION((Ce[vInd] <= config["Ce_lb"]*dbf_minus) & time_cond,# & (self.last_constraint()!=5),
            ...
            for vInd in range(Nvol['c']):
                    self.ON_CONDITION((Ce[-vInd-1] >= config["Ce_ub"]*dbf_plus) & time_cond,# & (self.last_constraint()!=5),
            ...
            for vInd in range(Nvol['a']):
                        for pInd in range(Npart['a']):
                            self.ON_CONDITION((self.particles['a'][vInd,pInd].cbar() >= config["Cs_ub"]*dbf_plus) & time_cond,
                                              switchToStates=[('Regulator',"Cs_limited_"+str(vInd)+"_"+str(pInd))],
                                              setVariableValues = [(self.time_counter, dae.Time()),
                                                                   (self.last_current, self.current()),
                                                                   (self.last_phi_applied, self.phi_applied()),
                                                                   (self.last_constraint,6+vInd*0.01+pInd*0.0001)])
            self.ON_CONDITION((Tavg >= config["T_ub"]*dbf_plus) & time_cond,# & (self.last_constraint()!=7),
            ...
            eq.Residual = self.phi_applied() - config["V_ub"]
            
            self.ON_CONDITION((self.current()  <= config["I_ub"]*dbf_plus) & time_cond,#  & (self.last_constraint()!=1),
                              switchToStates=[('Regulator','I_limited')],
                              setVariableValues = [(self.time_counter, dae.Time()),
                                                   (self.last_current, self.current()),
                                                   (self.last_phi_applied, self.phi_applied()),
            ...
            for ip in range(Npart['a']):  
                ...
                eq.Residual = self.particles['a'][Nvol['a']-1,ip].etaPlating() - config["LPOverpot_lb"]
            ...
            for iv in range(Nvol['a']):
                for ip in range(Npart['a']):
                    ...
                    eq.Residual = self.particles['a'][iv,ip].dcbardt()
